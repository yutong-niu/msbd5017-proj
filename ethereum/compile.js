...
function createConfiguration() {
    return {
        ...
        sources: {
            'DomainFactory.sol': {
                content: fs.readFileSync(path.resolve(__dirname, 'contracts', 'DomainFactory.sol'), 'utf8')
            },
        },
        settings: {
...
function getImports(dependency) {
    ...
    switch (dependency) {
        case 'Domain.sol':
            return {contents: fs.readFileSync(path.resolve(__dirname, 'contracts', 'Domain.sol'), 'utf8')};
        case 'common/Ownable.sol':
            return {contents: fs.readFileSync(path.resolve(__dirname, 'contracts', 'common', 'Ownable.sol'), 'utf8')};
        default:
...
function writeOutput(compiled, buildPath) {
    ...
    for (let contractFileName in compiled.contracts) {
        const contractName = path.basename(contractFileName).replace('.sol', '');
